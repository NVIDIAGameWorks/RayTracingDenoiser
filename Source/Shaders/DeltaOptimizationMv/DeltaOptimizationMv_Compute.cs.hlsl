...
#include "NRD.hlsli"
#include "STL.hlsli"
#include "DeltaOptimizationMv/DeltaOptimizationMv_Compute.resources.hlsli"
...
{
    return gIn_PrevDeltaSecondaryPosW.SampleLevel(gLinearClamp, samplePos * gInvRectSize, 0.f).xyz;
}

float LoadAngle(float2 samplePos, float3 primaryPosW, float3 refDir)
{
    float3 candidate = LoadCandidate(samplePos);
    float3 cDir = normalize(candidate - primaryPosW);
...
NRD_EXPORT void NRD_CS_MAIN(uint2 pixelPos : SV_DispatchThreadId)
{
    ...
    if (any(pixelPosUser >= gRectSize)) return;

    float3 initialMotion = gIn_ObjectMotion[pixelPosUser].xyz;
    float3 primaryPosW = gIn_DeltaPrimaryPosW[pixelPosUser].xyz;
    float3 secondaryPosW = gIn_DeltaSecondaryPosW[pixelPosUser].xyz;

    gOut_DeltaSecondaryPosW[pixelPosUser] = secondaryPosW;

    float2 pixelUv = float2(pixelPosUser + 0.5f) * gInvRectSize;
    float2 prevPixelUV = STL::Geometry::GetPrevUvFromMotion(pixelUv, primaryPosW, gWorldToClipPrev, initialMotion, gIsWorldSpaceMotionEnabled);
    ...
    if (length(initialScreenMotion) >= kMotionEpsilon)
    {
        ...
        float3 v = normalize(secondaryPosW - primaryPosW);

        float currAngle = LoadAngle(prevSamplePos, primaryPosW, v);
        ...
        for (uint i = 0; i < kMaxNewtonMethodIterations; ++i)
        {
            ...
            const float h = 1.f;
            float da_dx   = (LoadAngle(prevSamplePos + float2(h, 0), primaryPosW, v) - LoadAngle(prevSamplePos + float2(-h, 0), primaryPosW, v)) / (2.f * h);
            float da_dy   = (LoadAngle(prevSamplePos + float2(0, h), primaryPosW, v) - LoadAngle(prevSamplePos + float2(0, -h), primaryPosW, v)) / (2.f * h);
            float d2a_dx2 = (LoadAngle(prevSamplePos + float2(h, 0), primaryPosW, v) - 2 * currAngle + LoadAngle(prevSamplePos + float2(-h, 0), primaryPosW, v)) / (h * h);
            float d2a_dy2 = (LoadAngle(prevSamplePos + float2(0, h), primaryPosW, v) - 2 * currAngle + LoadAngle(prevSamplePos + float2(0, -h), primaryPosW, v)) / (h * h);
            float d2a_dxdy = (
                LoadAngle(prevSamplePos + float2(h, h), primaryPosW, v)
                - LoadAngle(prevSamplePos + float2(h, -h), primaryPosW, v)
                - LoadAngle(prevSamplePos + float2(-h, h), primaryPosW, v)
                + LoadAngle(prevSamplePos + float2(-h, -h), primaryPosW, v)
            ) / (4.f * h * h);
            ...
            uint iter = 0;
            while (LoadAngle(prevSamplePos + t * displacement, primaryPosW, v) > (currAngle + t * tmp2) && iter < kMaxLineSearchIterations)
            {
                t = beta * t;
                ++iter;
            }
