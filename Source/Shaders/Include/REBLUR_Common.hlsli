...
groupshared float3 s_Normal[ BUFFER_Y ][ BUFFER_X ];
...
float GetBlurRadius( float radius, float hitDist, float frustumHeight, float nonLinearAccumSpeed, float radiusBias, float radiusScale, float roughness = 1.0 )
{
...
float GetEncodingAwareRoughnessWeight( float roughnessCurr, float roughnessPrev, float fraction )
{
    float a = 1.0 / lerp( 0.01, 1.0, roughnessCurr * saturate( fraction ) );
...
{
    float a = 1.0 / lerp( 0.01, 1.0, roughness * fraction );
    float b = roughness * a;
