...
#include "REBLUR_Config.hlsli"
    ...
    #define NRD_DECLARE_INPUT_TEXTURES \
        ...
        NRD_INPUT_TEXTURE( Texture2D<float4>, gIn_History_Diff, t, 5 ) \
        NRD_INPUT_TEXTURE( Texture2D<float4>, gIn_HistoryFast_Diff, t, 6 ) \
        NRD_INPUT_TEXTURE( Texture2D<float4>, gIn_Spec, t, 7 ) \
        NRD_INPUT_TEXTURE( Texture2D<float4>, gIn_History_Spec, t, 8 ) \
        NRD_INPUT_TEXTURE( Texture2D<float4>, gIn_HistoryFast_Spec, t, 9 ) \
        EXTRA_INPUTS

    #define NRD_DECLARE_OUTPUT_TEXTURES \
        NRD_OUTPUT_TEXTURE( RWTexture2D<float>, gOut_ScaledViewZ, u, 0 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<unorm float4>, gOut_InternalData, u, 1 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<unorm float4>, gOut_Error, u, 2 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<float4>, gOut_Diff, u, 3 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<float4>, gOut_Fast_Diff, u, 4 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<float4>, gOut_Spec, u, 5 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<float4>, gOut_Fast_Spec, u, 6 )
    ...
    #define NRD_DECLARE_OUTPUT_TEXTURES \
        NRD_OUTPUT_TEXTURE( RWTexture2D<float>, gOut_ScaledViewZ, u, 0 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<unorm float2>, gOut_InternalData, u, 1 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<unorm float2>, gOut_Error, u, 2 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<float4>, gOut_Diff, u, 3 ) \
        NRD_OUTPUT_TEXTURE( RWTexture2D<float4>, gOut_Fast_Diff, u, 4 )
    ...
    #if( defined REBLUR_PROVIDED_CONFIDENCE )
        #define EXTRA_INPUTS \
            NRD_INPUT_TEXTURE( Texture2D<float>, gIn_SpecConfidence, t, 7 )
    #else
        #define EXTRA_INPUTS
    #endif
    ...
    #define NRD_DECLARE_INPUT_TEXTURES \
        ...
        NRD_INPUT_TEXTURE( Texture2D<float4>, gIn_History_Spec, t, 5 ) \
        NRD_INPUT_TEXTURE( Texture2D<float4>, gIn_HistoryFast_Spec, t, 6 ) \
        EXTRA_INPUTS
    ...
    #define NRD_DECLARE_OUTPUT_TEXTURES \
        ...
        NRD_OUTPUT_TEXTURE( RWTexture2D<float4>, gOut_Fast_Spec, u, 4 )
