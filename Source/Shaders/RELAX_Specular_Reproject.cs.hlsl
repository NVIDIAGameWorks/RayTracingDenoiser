...
#include "NRD.hlsl"
#include "STL.hlsl"
#include "RELAX_Specular_Reproject.resources.hlsl"
...
float loadSurfaceMotionBasedPrevData(
    ...
    out float prevSurfaceMotionBasedSpecular2ndMoment,
    ...
    out float historyLength)
{
    ...
    prevSurfaceMotionBasedSpecular2ndMoment = 0;
    ...
    if (any(bilinearTapsValid))
    {
        ...
        if (gUseBicubic && (bicubicFootprintValid > 0))
        {
            ...
            prevSurfaceMotionBasedSpecular2ndMoment =
                max(0, BicubicFloat(gPrevSpecular2ndMoment, gLinearClamp, prevPixelPosOnScreen, gInvViewSize));
        }
        else
        {
            ...
            prevSurfaceMotionBasedSpecular2ndMoment =
                max(0, BilinearWithBinaryWeightsFloat(gPrevSpecular2ndMoment, gLinearClamp, bilinearOrigin, bilinearWeights, bilinearTapsValid, interpolatedBinaryWeight));
        }

        // Always weighted Bilinear for responsive illumination
        BilinearWithBinaryWeightsLogLuv(prevSpecularResponsiveIllum, gPrevSpecularIlluminationResponsiveLogLuv, bilinearOrigin, bilinearWeights, bilinearTapsValid, interpolatedBinaryWeight);
        ...
        historyLength = 255.0 * BilinearWithBinaryWeightsFloat(gPrevHistoryLength, gLinearClamp, bilinearOrigin, bilinearWeights, bilinearTapsValid, interpolatedBinaryWeight);
    ...
    out float prevReflectionHitT)
{
    ...
    if (any(bilinearTapsValid))
    {
        ...
        prevSpecular2ndMoment = max(0, BilinearWithBinaryWeightsFloat(gPrevSpecular2ndMoment, gLinearClamp, bilinearOrigin, bilinearWeights, bilinearTapsValid, interpolatedBinaryWeight)).r;
...
NRD_EXPORT void NRD_CS_MAIN(int2 ipos : SV_DispatchThreadID, uint3 groupThreadId : SV_GroupThreadID, uint3 groupId : SV_GroupID)
{
    ...
    float  prevSurfaceMotionBasedSpecular2ndMoment;
    ...
    float currentSpecM2 = lerp(prevSurfaceMotionBasedSpecular2ndMoment, specular2ndMoment, specSurfaceAlpha);
    ...
    gOutSpecularIlluminationLogLuv[ipos] = STL::Color::LinearToLogLuv(accumulatedSpecularIllumination);
    gOutSpecularIlluminationResponsiveLogLuv[ipos] = STL::Color::LinearToLogLuv(accumulatedSpecularIlluminationResponsive);

    gOutSpecular2ndMoment[ipos] = accumulatedSpecular2ndMoment;

    gOutReflectionHitT[ipos] = accumulatedReflectionHitT;

    gOutHistoryLength[ipos] = historyLength / 255.0;
