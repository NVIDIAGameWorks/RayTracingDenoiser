...
#include "NRD.hlsli"
#include "STL.hlsli"
#include "RELAX_Specular_Reproject.resources.hlsli"
...
float GetSpecAccumSpeed(float maxAccumSpeed, float roughness, float NoV, float parallax)
{
    ...
    float powerScale = 1.0 + parallax * 3.0 * parallaxSensitivity;
    ...
    if (any(bilinearTapsValid))
    {
        ...
        if (bicubicFootprintValid > 0)
        {
            ...
            prevSpecularIllumAnd2ndMoment =
                max(0, BicubicFloat4(gPrevSpecularIllumination, gLinearClamp, prevPixelPosOnScreen, gInvResourceSize));
#if( RELAX_USE_BICUBIC_FOR_FAST_HISTORY == 1 )
            prevSpecularResponsiveIllum = max(0, BicubicFloat4(gPrevSpecularIlluminationResponsive, gLinearClamp, prevPixelPosOnScreen, gInvResourceSize)).rgb;
#else
        ...
        historyLength = 255.0 * BilinearWithBinaryWeightsFloat(gPrevSpecularHistoryLength, bilinearOrigin, bilinearWeights, bilinearTapsValid, interpolatedBinaryWeight);
...
NRD_EXPORT void NRD_CS_MAIN(int2 ipos : SV_DispatchThreadId, uint3 groupThreadId : SV_GroupThreadId, uint3 groupId : SV_GroupId)
{
    ...
    gOutSpecularHistoryLength[ipos] = historyLength / 255.0;
    ...
    float dHitT = max(0, sqrt(abs(prevReflectionHitTVMB - prevReflectionHitTSMB)) - 1.0 * specSigma.a);
