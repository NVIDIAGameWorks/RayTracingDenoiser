...
// Inputs
NRI_RESOURCE( Texture2D<uint>, gIn_InternalData, t, 0, 0 );
...
void main( uint2 pixelPos : SV_DispatchThreadId )
{
    ...
    #if( SHOW_MIPS != 0 )
    {
        ...
        float2 mipUv = pixelUv * float2( gScreenSize ) / ( mipSize * float( 1 << realMipLevel ) );
            ...
            float4 a00 = gIn_DiffA.SampleLevel( gNearestClamp, mipUvFootprint00, mipLevel, int2( 0, 0 ) );
            ...
            float4 b00 = gIn_DiffB.SampleLevel( gNearestClamp, mipUvFootprint00, mipLevel, int2( 0, 0 ) );
    ...
    float normAccumSpeed = saturate( internalData.x * STL::Math::PositiveRcp( internalData.y * HISTORY_FIX_FRAME_NUM_PERCENTAGE ) ); // .x instead of .z in specular to get free upsampling if maxAccumulatedFrameNum < 5 (it doesn't work for specular)
    float realMipLevelf = GetMipLevel( normAccumSpeed, internalData.z );
    uint realMipLevel = uint( realMipLevelf );

    [branch]
    if ( realMipLevel == 0 || USE_HISTORY_FIX == 0 )
        return;
    ...
    while( sum == 0.0 && realMipLevel != 0 )
    {
        ...
        for( int i = 0; i < 2; i++ )
        {
            ...
            for( int j = 0; j < 2; j++ )
            {
                ...
                float4 a00 = gIn_DiffA.SampleLevel( gNearestClamp, uv, mipLevel, int2( 0, 0 ) );
                ...
                float4 b00 = gIn_DiffB.SampleLevel( gNearestClamp, uv, mipLevel, int2( 0, 0 ) );
    ...
    blurryB *= invSum;
        ...
        blurryA = lerp( blurryA, originalA, normAccumSpeed );
        blurryB.xyz = lerp( blurryB.xyz, originalB.xyz, normAccumSpeed );
    #endif
