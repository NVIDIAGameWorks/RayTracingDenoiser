...
void Preload( uint2 sharedPos, int2 globalPos )
{
    ...
    s_Normal_Roughness[ sharedPos.y ][ sharedPos.x ] = NRD_FrontEnd_UnpackNormalAndRoughness( gIn_Normal_Roughness[ gRectOrigin + globalPos ] );
...
NRD_EXPORT void NRD_CS_MAIN( int2 threadPos : SV_GroupThreadId, int2 pixelPos : SV_DispatchThreadId, uint threadIndex : SV_GroupIndex )
{
    ...
    // Diffuse
        ...
        float diffResultClamped = min( diffResult, diffHistory * REBLUR_MAX_FIREFLY_RELATIVE_INTENSITY );
    ...
    // Specular
        ...
        float specResultClamped = min( specResult, specHistory * REBLUR_MAX_FIREFLY_RELATIVE_INTENSITY );
