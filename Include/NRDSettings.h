...
namespace nrd
{
    ...
    */
    struct CommonSettings
    {
        ...
        float denoisingRange = 1e7f;                    // (m) > 0, use TLAS or tracing range
    ...
    */
    ...
    struct RelaxSpecularSettings
    {
        ...
        float phiNormal = 64.0f;                                    // A-trous edge stopping normal sensitivity for diffuse, spatial variance estimation normal sensitivity
        ...
        float specularLobeAngleSlack = 0.3f;                        // slack (in degrees) for the specular lobe angle used in normal based rejection of specular during A-Trous passes
    ...
    struct RelaxDiffuseSpecularSettings
    {
        ...
        bool antifirefly = false;                                   // firefly suppression
        CheckerboardMode checkerboardMode = CheckerboardMode::OFF;  // If not OFF, diffuse mode equals checkerboard mode set here, and specular mode opposite: WHITE if diffuse is BLACK and vice versa
    };
}
